...
class ModelExtensionModulePromSync extends Model
{
    ...
    private function buildProductSeoUrl(array $prom_product, array $languages, $oc_product_id = 0)
    {
        $seo = array();
        $store_id = 0;
        $used_keywords = array();
        ...
        foreach ($languages as $language) {
            ...
            // Check if this keyword was already generated for another language in this product, add prefix
            if (in_array($keyword, $used_keywords)) {
                $keyword = $prefix . '-' . $keyword;
            }

            // Check for conflict in DB
            $check = $this->db->query("SELECT seo_url_id FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" . $this->db->escape($keyword) . "'" . ($oc_product_id ? " AND query != 'product_id=" . (int) $oc_product_id . "'" : "") . " LIMIT 1");
            if ($check->num_rows) {
                // Even if it didn't conflict internally, it conflicts with another product
                if (!preg_match('/^' . $prefix . '-/', $keyword)) {
                    $keyword = $prefix . '-' . $keyword;
                }
            }
    ...
    public function fixSeoUrls()
    {
        ...
        foreach ($languages as $l) {
            $code = strtolower(substr($l['code'], 0, 2));
            if ($code === 'ua')
                $code = 'uk';
            $oc_languages[$l['language_id']] = $code;
        }

        $query = $this->db->query("SELECT oc_product_id FROM `" . DB_PREFIX . "prom_sync_product` ORDER BY oc_product_id DESC");
        $products = $query->rows;

        $fixed = 0;
        foreach ($products as $p) {
            $product_id = (int) $p['oc_product_id'];
            $seo_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE query = 'product_id=" . $product_id . "'");

            $product_keywords = array();
            foreach ($seo_query->rows as $row) {
                $product_keywords[$row['language_id']] = $row;
            }

            foreach ($product_keywords as $lang_id => $row) {
                $keyword = $row['keyword'];
                $seo_url_id = (int) $row['seo_url_id'];

                $conflict_other = $this->db->query("SELECT seo_url_id FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" . $this->db->escape($keyword) . "' AND query != 'product_id=" . $product_id . "' LIMIT 1");

                $conflict_internal = false;
                foreach ($product_keywords as $other_lang_id => $other_row) {
                    if ($other_lang_id != $lang_id && $other_row['keyword'] === $keyword) {
                        $conflict_internal = true;
                        break;
                    }
                }

                if ($conflict_other->num_rows || $conflict_internal) {
                    $prefix = isset($oc_languages[$lang_id]) ? $oc_languages[$lang_id] : 'p';
                    ...
                    $new_keyword = $prefix . '-' . $keyword;
                    $this->db->query("UPDATE `" . DB_PREFIX . "seo_url` SET keyword = '" . $this->db->escape($new_keyword) . "' WHERE seo_url_id = '" . $seo_url_id . "'");

                    $product_keywords[$lang_id]['keyword'] = $new_keyword;
                    $fixed++;
                }
            }
        }
