...
class ControllerExtensionModulePromSync extends Controller
{
    ...
    public function index()
    {
        ...
        $data['token_key'] = !empty($this->session->data['user_token']) ? 'user_token' : 'token';
        ...
        $data['entry_cron_url_inventory'] = $this->language->get('entry_cron_url_inventory');
        ...
        if ($catalog) {
            ...
            $data['cron_url_inventory'] = $catalog . 'index.php?route=extension/module/prom_sync/cron_inventory&key=' . $cron_key;
        } else {
            $data['cron_url'] = 'index.php?route=extension/module/prom_sync/cron&key=' . $cron_key;
            $data['cron_url_inventory'] = 'index.php?route=extension/module/prom_sync/cron_inventory&key=' . $cron_key;
        }

        $data['fix_seo_url'] = $this->url->link('extension/module/prom_sync/fix_seo_urls', 'user_token=' . $this->session->data['user_token'], true);
        $data['text_section_tools'] = $this->language->get('text_section_tools');
        $data['entry_fix_seo'] = $this->language->get('entry_fix_seo');
        $data['button_fix_seo'] = $this->language->get('button_fix_seo');
        $data['text_section_sync_fields'] = $this->language->get('text_section_sync_fields');
        $data['help_sync_fields'] = $this->language->get('help_sync_fields');
        $data['help_fix_seo'] = $this->language->get('help_fix_seo');
    ...
    public function fix_seo_urls()
    {
        ...
        $this->load->model('extension/module/prom_sync');
        $fixed = $this->model_extension_module_prom_sync->fixSeoUrls();

        $this->load->language('extension/module/prom_sync');

        $this->response->setOutput(json_encode(array(
            'success' => true,
            'message' => sprintf($this->language->get('text_fix_seo_success'), $fixed)
        )));
    }
